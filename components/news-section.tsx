    ...
    <section className="py-20 px-4 sm:px-6 lg:px-8 bg-gradient-to-br from-slate-900 to-slate-800">
      <div className="max-w-7xl mx-auto">
        <div className="grid grid-cols-1 lg:grid-cols-[minmax(0,1fr)_minmax(360px,460px)] gap-10 lg:gap-16 items-center">
          ...
          >
            ...
            <div className="flex flex-wrap items-center gap-2 text-white">
              <button
                ...
                disabled={page === 0}
                aria-label="前のページへ"
                className="flex h-10 w-10 items-center justify-center rounded-full border border-white/15 text-sm font-medium transition disabled:cursor-not-allowed disabled:opacity-40 hover:bg-white/10"
              >
                {"<"}
              </button>
          ...
          <div
            ref={imageRef}
            className={`relative w-full max-w-[460px] lg:justify-self-end min-h-[560px] rounded-3xl overflow-hidden transition-all duration-1000 transform ${
              imageVisible
                ? "opacity-100 translate-y-0"
                : "opacity-0 translate-y-10"
            }`}
          >
            <div className="flex w-full h-full justify-center bg-slate-800 border border-slate-600 rounded-3xl overflow-hidden p-3">
              <blockquote
